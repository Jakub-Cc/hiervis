...
public class MultiIntField extends JTextField {
	private static final Logger log = LogManager.getLogger(MultiIntField.class);
	...
	private void validateText() throws Exception {
		...
		if (txt.isEmpty()) {
			throw new ValidationException("Value mandatory");
		}
		...
		for (String value : values) {
			try {
				...
				if (val < min) {
					throw new ValidationException("\"" + val + "\" must be biger that minimum - " + min);
				} else if (val > max) {
					throw new ValidationException("\"" + val + "\" must be smaller that maximum - " + max);
				} else {
					valueList.add(val);
				}
			} catch (NumberFormatException e) {
				throw new ValidationException("\"" + value + "\" could not be parsed to a value");
			}
		}
		if (valueList.isEmpty()) {
			throw new ValidationException("\"" + txt + "\" could not be parsed to a value");
		}
	...
	private boolean charIsNumberOrColon(char character) {
		return character == '1' || character == '2' || character == '3' || character == '4' || character == '5'
