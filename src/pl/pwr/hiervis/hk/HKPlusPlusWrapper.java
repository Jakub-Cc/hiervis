...
public class HKPlusPlusWrapper {
    ...
    /**
     * Creates and starts the subprocess with the specified arguments, and creates a wait
     * dialog at the specified owner window.
     * 
     * @param owner               frame at which the wait dialog will be created.
     * @param trueClassAttribute  indicates that FIRST column of data is true class attribute,
     *                            class should be indicated by string.
     * @param instanceNames       indicates that SECOND (if class attribute is present) or
     *                            FIRST (otherwise) column is the name of every instance.
     * @param diagonalMatrix      use simple diagonal matrix instead of full matrix as a
     *                            covariance matrix
     * @param disableStaticCenter disable feature of placing static (background) center while
     *                            going down in hierarchy
     * @param generateImages      store clusterisation results also as images (ONLY first two
     *                            dimensions are visualized!). The dimension of each image is
     *                            set to 800x800.
     * @param epsilon             epsilon value expressed as 10^-epsilon, used in comparing
     *                            values to 0.0, reducing round-off error. Default value is
     *                            10.
     * @param littleValue         value of diagonal matrix elements expressed as
     *                            10^-littleValue, used in forcing covariance matrix to be
     *                            non-singular. Default value is 5.
     * @param clusters            number of clusters generated by clusterisation algorithm
     * @param iterations          number of maximum iterations made by clusterisation
     *                            algorithm
     * @param repeats             number of algorithm repeats (new initialization of clusters)
     ...
     */
    public void start(Window owner, boolean trueClassAttribute, boolean instanceNames, boolean diagonalMatrix, boolean disableStaticCenter,
	    boolean generateImages, int epsilon, int littleValue, int clusters, int iterations, int repeats, int dendrogramSize, int maxNodeCount,
	...
	// files in one place
	process = new ProcessBuilder(buildArgsList(trueClassAttribute, instanceNames, diagonalMatrix, disableStaticCenter, generateImages, epsilon,
		littleValue, clusters, iterations, repeats, dendrogramSize, maxNodeCount, verbose)).redirectErrorStream(true).directory(hkOutDir)
			.start();
    ...
    /**
     * Prepares the input file that the HK subprocess will load by serializing the specified
     * hierarchy and saving it in the file that HK is configured to load.
     * 
     * @param hierarchy         the source hierarchy
     * @param selectedNode      the node in the specified hierarchy denoting the subtree that
     *                          is to be serialized to the file
     * @param withTrueClass     whether the input file should include true class column
     * @param withInstanceNames whether the input file should include instance name column
     * @throws IOException if an I/O error occurs
     */
    public void prepareInputFile(Hierarchy hierarchy, Node selectedNode, boolean withTrueClass, boolean withInstanceNames) throws IOException {
     ...
     * 
     * @param withTrueClass     whether the output file should be read assuming that it
     *                          contains true class column
     * @param withInstanceNames whether the output file should be read assuming that it
     *                          contains instance names column
     * @param useSubtree        whether nodes should be created with 'useSubtree' option
	...
	LoadedHierarchy.Options options = new LoadedHierarchy.Options(withInstanceNames, withTrueClass, false, false, useSubtree);

	Hierarchy output = new GeneratedCSVReader().load(hkOutputFile.getAbsolutePath(), options.hasInstanceNameAttribute,
		options.hasTrueClassAttribute, options.hasColumnHeader, options.isFillBreadthGaps, options.isUseSubtree);
     ...
     * 
     * @param trueClassAttribute  indicates that FIRST column of data is class attribute,
     *                            class should be indicated by string.
     * @param instanceNames       indicates that SECOND (if class attribute is present) or
     *                            FIRST (otherwise) column is the name of every instance.
     * @param diagonalMatrix      use simple diagonal matrix instead of full matrix as a
     ...
     */
    private List<String> buildArgsList(boolean trueClassAttribute, boolean instanceNames, boolean diagonalMatrix, boolean disableStaticCenter,
	    boolean generateImages, int epsilon, int littleValue, int clusters, int iterations, int repeats, int dendrogramSize, int maxNodeCount,
	    boolean verbose) {
