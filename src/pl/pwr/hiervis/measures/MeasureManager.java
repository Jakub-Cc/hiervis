...
/**
 * Class for managing measures, {@link MeasureTask}s, and data/processes related
 * to them.
 * 
...
public class MeasureManager {
	...
	private static final Logger log = LogManager.getLogger(MeasureManager.class);
	 ...
	 * 
	 * @param h       the hierarchy to check the measure for
	 ...
	 * @param h       the hierarchy to compute the measure for
	 * @param measure the measure to post
	 */
	...
	/**
	 * Posts all measure tasks that have been defined as auto-compute for the
	 * specified hierarchy.
	 * 
	 * @param holder the result holder in which the measure result will be saved
	 * @param h      the hierarchy to compute the measures for
	 */
	public void postAutoComputeTasksFor(MeasureResultHolder holder, Hierarchy h) {
		for (MeasureTask task : getAllMeasureTasks()) {
			if (task.autoCompute && task.applicabilityFunction.apply(h) && !holder.isMeasureComputed(h, task)) {
	 ...
	 * 
	 * @param lh      the hierarchy to remove the measure for
	 ...
	 * 
	 * @param dirPath the directory containing all {@link MeasureTask} script files.
		...
		try (Stream<Path> files = Files.walk(dirPath)) {
			files.forEach(filePath -> {
				if (Files.isRegularFile(filePath, LinkOption.NOFOLLOW_LINKS)) {
					...
					if (measure != null) {
						String groupPath = filePath.getParent().toString().replace(dirPath.toString(), "").replace("\\",
								"/");
	...
	/**
	 * @param groupId id of the measure group we want to receive
	 * @return returns an unmodifiable collection of all measures belonging to the
	...
	/**
	 * @param predicate the predicate that {@link MeasureTask}s have to match in
	...
	public Collection<MeasureTask> getMeasureTasks(Predicate<MeasureTask> predicate) {
		return measureGroupMap.values().stream().flatMap(Collection::stream).filter(predicate)
	...
	/**
	 * @return a sorted collection of ids of all measure groups that have been
	 *         loaded.
	 */
	public Collection<String> listMeasureTaskGroups() {
		return measureGroupMap.keySet().stream().sorted().collect(Collectors.toList());
	}
	...
	public void dumpMeasures(Path destinationFile, LoadedHierarchy hierarchy) {
		final Function<Object, String> resultToCSV = data -> {
			if (data instanceof Number) {
				...
				throw new IllegalArgumentException(
						"Unexpected data type in measure result: " + data.getClass().getName());
			}
		};

		final Function<MeasureTask, String> dumpHistogram = task -> {
			StringBuilder buf2 = new StringBuilder();
			Object measureResult = hierarchy.measureHolder.getMeasureResultOrDefault(hierarchy.getMainHierarchy(), task,
					new double[0]);
		...
		measures.forEach(task -> {
			Object measureResult = hierarchy.measureHolder.getMeasureResult(hierarchy.getMainHierarchy(), task);
