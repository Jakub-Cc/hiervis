...
public class HistogramTable extends TableEx {
	...
	protected HashMap<String, Double> mBinMin = new HashMap<>();
	protected HashMap<String, Double> mBinMax = new HashMap<>();

	protected HashMap<String, Integer> mCountMins = new HashMap<>();
	protected HashMap<String, Integer> mCountMaxes = new HashMap<>();
	protected double mBinWidth;
	protected int mBinCount;
	...
	public HistogramTable(Table aTable, int aBinCount) {
		...
		mBinCount = aBinCount;
		initializeHistogramTable(fieldNames, mBinCount);
	...
	private void initializeStringColumn(String field, Column dataColumn) {
		...
		mCountMins.put(field, 0);
		mCountMaxes.put(field, 0);
		...
		mBinMin.put(field, 0.0);
		mBinMax.put(field, (double) values.size());
		...
		while (keys.hasMoreElements() && rowIndex < mBinCount) {
	...
	private void initializeNumericBinInfo(String field, Column dataColumn) {
		...
		mBinMax.put(field, maxValue);
		mBinMin.put(field, minValue);

		mBinWidth = (1 + maxValue - minValue) / mBinCount;

		assert mBinWidth >= 0.0 : "m_binWidth < 0!";
	}
	...
	private void initializeNumericBinColumn(String field) {
		double dataColumnMin = mBinMin.get(field);
		for (int binIndex = 0; binIndex < mBinCount; binIndex++) {
			set(binIndex, field, dataColumnMin + binIndex * mBinWidth);
		}
	}
	...
	private void initializeCountColumn(String field, Column dataColumn) {
		...
		for (int dataRowIndex = 0; dataRowIndex < dataColumn.getRowCount(); dataRowIndex++) {
			...
			binSlot = (int) ((cellValue - dataColumnMin) / mBinWidth);
	...
	public double getBinMin(String field) {
		return mBinMin.get(field);
	}

	public double getBinMax(String field) {
		return mBinMax.get(field);
	}
	...
	public double getCountMin(String field) {
		...
		if (null == mCountMaxes.get(countField)) {
			minMax = getNumericColumnMinMax(countField);
			mCountMins.put(countField, (int) minMax[0]);
			mCountMaxes.put(countField, (int) minMax[1]);
		}

		return mCountMins.get(countField);
	}
	...
	public double getCountMax(String field) {
		...
		return mCountMaxes.get(countField);
	}
