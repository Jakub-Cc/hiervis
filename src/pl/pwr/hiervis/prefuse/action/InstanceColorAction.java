...
public class InstanceColorAction extends ColorAction {
	...
	/**
	 * @param elementRole the {@link ElementRole} to test for
	 * @return creates and returns a predicate which returns true for instances
	 *         whose node's {@link ElementRole} is the same as the one passed in
	 *         argument.
	 */
	public static ComparisonPredicate getPredicateFor(ElementRole elementRole) {
		return new ComparisonPredicate(ComparisonPredicate.EQ, new InstanceNodeExpression(),
				Literal.getLiteral(elementRole.getNumber()));
	}

	/**
	 * Given a row from the instance data table, extracts the node to which that
	 * instance belongs and returns its {@link ElementRole}.
	 */
