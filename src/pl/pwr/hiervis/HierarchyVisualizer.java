...
public final class HierarchyVisualizer {
	...
	if (cmd.hasOption('i')) {
	    ...
	    loadOptions = new LoadedHierarchy.Options(withInstanceNames, withTrueClass, withHeader, fixBreadthGaps, useSubtree);
	...
	Option helpOpt = OptionBuilder.hasArg(false).isRequired(false).withDescription("prints this message").withLongOpt("help").create('h');
	...
	if (SystemUtils.IS_OS_UNIX && SwingUIUtils.isXFCE() && SwingUIUtils.isOpenJDK()) {
		...
		if (config.isStopXfceLafChange()) {
			...
			prefLAF = Arrays.stream(UIManager.getInstalledLookAndFeels())
				.filter(laf -> !laf.getName().toLowerCase(Locale.ENGLISH).equals("metal")).findFirst().get().getName();
	...
	if (prefLAF != null && !prefLAF.isEmpty()) {
		...
		LookAndFeelInfo lafInfo = Arrays.stream(UIManager.getInstalledLookAndFeels()).filter(laf -> laf.getName().equals(fprefLAF))
			.findFirst().get();
		...
		log.printf(Level.ERROR, "Could not find a matching L&F for name '%s'. Falling back to system default.", prefLAF);
	    }
	    catch (Exception ex) {
	    ...
	    if (!successLAF) {
		log.printf(Level.INFO, "Could not find a matching L&F for name '%s'. Falling back to system default.", prefLAF);
	    }
	}
	...
	try {
	    // Due to some internal workings of Swing, default sounds are not played for non-default
	    // L&Fs.
	...
	PatternLayout layout = PatternLayout.createLayout("%d{HH:mm:ss.SSS} [%t] %-5level %logger{36} - %msg%throwable%n", null, null, null,
		Charset.defaultCharset(), false, false, null, null);

	FileAppender appender = FileAppender.createAppender("logs/log-" + subtitle + ".txt", "false", "false", "LogFile-" + subtitle, "true", "true",
		"true", "8192", layout, null, "false", "", config);
    ...
    public static void spawnNewInstance(String subtitle, File inputFile, boolean withTrueClass, boolean withInstanceNames) throws IOException {
