...
public class DockerUI extends JFrame implements DockingConstants {
	...
	DimensionReductionWrapInstanceVisualizationsFrame wraper = new DimensionReductionWrapInstanceVisualizationsFrame(context);
	...
	mntmCut.addActionListener(e -> {
	    String nodeName = context.getHierarchy().getTree().getNode(context.getHierarchy().getSelectedRow()).get(0).toString();
	    String tabTitle = "[" + nodeName + "] " + context.getHierarchyFrame().getSelectedTabTitle();
	    Hierarchy cutHierarchy = HierarchyUtils.subHierarchy(context.getHierarchy().getHierarchyWraper().getOriginalHierarchy(), nodeName,
		    (String) null);
	...
	// TODO zrobic inne okno bo to srednio sobie radzi z grabem fokusa tzn mouse click na ok
	// albo aby przejmowalo fokus
