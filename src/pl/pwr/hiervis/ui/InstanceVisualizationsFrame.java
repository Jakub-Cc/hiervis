...
public class InstanceVisualizationsFrame extends JFrame {
	...
	private static final int DEFAULT_LABEL_HEIGHT = new JLabel(" ").getPreferredSize().height;
	private static final int VIS_WIDTH_MIN = 100;
	private static final int VIS_WIDTH_MAX = 1000;
	private static final int VIS_HEIGHT_MIN = 100;
	private static final int VIS_HEIGHT_MAX = 1000;
	...
	private void createVisualizationHolder() {
		...
		scrollPane.addMouseWheelListener(this::mouseWheelMoved);
	}

	private void mouseWheelMoved(MouseWheelEvent e) {
		Display dis = getFirstVisibleDisplay();
		if (e.isControlDown() && dis != null) {
			...
			visWidth = Utils.clamp(VIS_WIDTH_MIN, visWidth, VIS_WIDTH_MAX);
			visHeight = Utils.clamp(VIS_HEIGHT_MIN, visHeight, VIS_HEIGHT_MAX);
	...
	private void recreateCheckboxes(String[] dataNames) {
		...
		for (int i = 0; i < dims; ++i) {
			...
			cboxH.addActionListener(e -> updateToggleAll(true));
			cboxV.addActionListener(e -> updateToggleAll(false));
	...
	private GridBagLayout createLabelLayout(int totalDims, boolean horizontal) {
		...
		if (horizontal) {
			...
			layout.rowHeights = new int[] { DEFAULT_LABEL_HEIGHT };
			layout.rowWeights = new double[] { 0.0 };
		} else {
			layout.rowHeights = new int[totalDims];
			layout.rowWeights = new double[totalDims];

			layout.columnWidths = new int[] { DEFAULT_LABEL_HEIGHT };
	...
	private DisplayEx createInstanceDisplayFor(int dimX, int dimY) {
		...
		return createInstanceDisplayFor(vis, dimX, dimY);
	}
	...
	private void forEachScatterPlot(Consumer<DisplayEx> func) {
		for (Component c : cViewport.getComponents()) {
			if (c instanceof DisplayEx && !(c instanceof HistogramGraph)) {
				func.accept((DisplayEx) c);
			}
		}
	}
	 ...
	 */
	private void updateToggleAll(boolean horizontal) {
		JCheckBox[] arr = horizontal ? cboxesHorizontal : cboxesVertical;
		...
		if (cboxToggle.isSelected()) {
			...
			setCheckboxSilent(cboxToggle, Arrays.stream(arr).allMatch(JCheckBox::isSelected));
		}
	}
