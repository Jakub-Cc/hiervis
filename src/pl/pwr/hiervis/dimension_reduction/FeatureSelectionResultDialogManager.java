...
import pl.pwr.hiervis.dimension_reduction.ui.FeatureSelectionResultDialog;
...
public class FeatureSelectionResultDialogManager {
	...
	private void deleteDialogIfExisting(LoadedHierarchy loadedHierarchy, FeatureSelection featureSelection) {
		Predicate<ResultDialog> loadedHierPred = resultDialog -> resultDialog.loadedHierarchy.equals(loadedHierarchy);
		Predicate<ResultDialog> fsPred = resultDialog -> resultDialog.featureSelection.getClass()
				.equals(featureSelection.getClass());
	...
	private FeatureSelectionResultDialog getResultDialog(LoadedHierarchy loadedHierarchy,
			Class<?> featureSelectionClass) {
		for (ResultDialog resultDialog : list) {
			if (resultDialog.loadedHierarchy == loadedHierarchy
					&& resultDialog.featureSelection.getClass() == featureSelectionClass) {
	...
	public void addResultAndShow(CalculatedDimensionReduction reduction) {
		deleteDialogIfExisting(reduction.getInputLoadedHierarchy(),
				(FeatureSelection) reduction.getDimensionReduction());
		FeatureSelectionResultDialog dialog = addDialog(reduction.getInputLoadedHierarchy(),
				(FeatureSelection) reduction.getDimensionReduction(), reduction.getFsResult());
	...
	private class ResultDialog {
		...
		public ResultDialog(LoadedHierarchy loadedHierarchy, FeatureSelection featureSelection,
				FeatureSelectionResultDialog dialog) {
