...
public class HierarchyLoaderThread extends Thread {
	...
	/**
	 * 
	 * @param config HVConfig object describing parameters to use while loading the
	 *               file
	 * @param file   the file to load (CSV format)
	 */
	...
	public void run() {
		...
		try {
			...
			Hierarchy hierarchy = reader.load(file.getAbsolutePath(), options.hasInstanceNameAttribute,
					options.hasTrueClassAttribute, options.hasColumnHeader, options.isFillBreadthGaps,
	...
	/**
	 * Perform various tests on the hierarchy, meant to detect whether the input
	 * file has been loaded with correct options selected.
	 * 
	 * @param h the hierarchy to verify
	 * @throws InvalidAlgorithmParameterException
	 */
	private void verify(Hierarchy h) throws InvalidAlgorithmParameterException {
		int dataDims = h.getRoot().getSubtreeInstances().getFirst().getData().length;

		if (dataDims == 1) {
			throw new InvalidAlgorithmParameterException(
					"Instance data only has 1 feature. Minimum of 2 are required.");
		}
		...
		if (dataNames != null) {
			for (int i = 0; i < dataNames.length; ++i) {
				try {
					...
					// If it failed to parse as double, then it's likely some identifier. All's
					// good.
				}
			}
		}
