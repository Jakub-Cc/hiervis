...
/**
 * This class contains methods that are used to show customizable dialogs and UI
 * prompts to the user.
 * 
...
public final class SwingUIUtils {
	...
	/**
	 * <p>
	 * Installs a close action in the specified dialog. This action sends the window
	 * closing event, so all listeners should be correctly triggered.
	 * </p>
	...
	/**
	 * <p>
	 * Installs a close action in the specified frame. This action sends the window
	 * closing event, so all listeners should be correctly triggered.
	 * </p>
	 * - Bound to the Escape key.<br/>
	...
	public static void installEscapeCloseOperation(final JFrame frame) {
		installOperation(frame, JComponent.WHEN_IN_FOCUSED_WINDOW, KS_ESCAPE, AMK_DISPATCH_WINDOW_CLOSING,
				new AbstractAction(AMK_DISPATCH_WINDOW_CLOSING) {
					public void actionPerformed(ActionEvent e) {
	...
	/**
	 * Attaches an empty action to the specified keybind, so that its default
	 * behaviour can be disabled without having to modify the parent's action.
	 * 
	 * 
	 * @see #AMK_IGNORE_OPERATION
	 */
	public static void installIgnoredOperation(final JDialog dialog, final int condition, final KeyStroke keyStroke) {
	 ...
	 * 
	 * @param frame     The frame in which this keybind can be activated
	 * @param condition When should this keybind be activated. Either
	 *                  {@link JComponent#WHEN_FOCUSED},
	 *                  {@link JComponent#WHEN_IN_FOCUSED_WINDOW}, or
	 *                  {@link JComponent#WHEN_ANCESTOR_OF_FOCUSED_COMPONENT}.
	 * @param keyStroke The keystroke used to activate the keybind
	 * @param actionKey Identifier of the action
	 * @param action    The action to execute when the keybind is activated
	 */
	public static void installOperation(final RootPaneContainer frame, final int condition, final KeyStroke keyStroke,
			final String actionKey, Action action) {
	 ...
	 */
	public static Action installOperation(final RootPaneContainer frame, final int condition, final KeyStroke keyStroke,
	 ...
	 * 
	 * @param frame     The frame from which the keybind is to be uninstalled.
	 * @param condition When should this keybind be activated. Either
	 *                  {@link JComponent#WHEN_FOCUSED},
	 *                  {@link JComponent#WHEN_IN_FOCUSED_WINDOW}, or
	 *                  {@link JComponent#WHEN_ANCESTOR_OF_FOCUSED_COMPONENT}.
	 * @param keyStroke The keystroke used to activate the keybind
	 */
	public static void uninstallOperation(final RootPaneContainer frame, final int condition,
	...
	/**
	 * Returns the Action installed under the specified action key in the specified
	 * frame.
	 * 
	 * @param frame     The frame in which the action is installed
	 * @param actionKey The action key to which the action is bound
	 * @param selfOnly  If true, will only check the frame specified in argument for
	 *                  actions bound to the action key. If false, will check any
	 *                  parents of the action map for actions bound to the action
	 *                  key, if none was found in the first one.
	 */
	...
	/**
	 * @param component the component whose index is to be returned
	...
	/**
	 * Creates and registers a {@link WindowListener} to {@code parent} window,
	 * which causes the {@code child} window to close when the {@code parent} window
	 * is closed.
	 * 
	 * @param parent the window whose closing will cause child to close as well.
	 * @param child  the window that will be closed when parent is closed.
	...
	public static WindowListener addCloseCallback(Window parent, Window child) {
		...
		return addCloseCallback(parent, () -> child.dispatchEvent(new WindowEvent(child, WindowEvent.WINDOW_CLOSING)));
	}

	/**
	 * Creates and registers a {@link WindowListener} to {@code window}, which
	 * executes the callback expression.
	 * 
	 * @param window   the window whose closing will cause the callback to be
	 *                 executed.
	 * @param callback the callback to execute when the window closes.
	 * @return the created listener
	 */
	...
	/**
	 * Executes backgroundTask in the background asynchronously, and displays a
	 * dialog to inform the user that the operation is being performed.
	 * 
	 * Does not support progress or current operation status.
	 * 
	 * @param window         the window that serves as owner of the wait dialog
	 * @param title          title of the wait dialog
	 * @param log            logger for errors that occur during the task
	 * @param modal          whether the wait dialog should be modal, ie. prevent
	 *                       the user from interacting with the rest of the
	 *                       application while the dialog is visible
	 * @param backgroundTask the task to perform in the background
	 * @param doneTask       the task to perform if the backgroundTask succeeds. Can
	 *                       be null.
	 * @param failTask       the task to perform if the backgroundTask fails. Can be
	 *                       null.
	 */
	public static void executeAsyncWithWaitWindow(Window window, String title, Logger log, boolean modal,
	...
	public static boolean isXFCE() {
		...
		String[] vars = { getAnyVar("XDG_CURRENT_DESKTOP"), getAnyVar("GDMSESSION"), getAnyVar("DESKTOP_SESSION"),
	...
	/**
	 * @return true if the Java version installed on the user's system is identified
	 *         as OpenJDK (lowercased version string contains 'open jdk' or
	 *         'openjdk')
	 */
	public static boolean isOpenJDK() {
		...
		if (vmName == null)
			return false; // Can't tell, assume it's not OpenJDK.
	...
	/**
	 * Attempts to retrieve the value of the specified system or environment
	 * variable, or its uppercase / lowercase variants (in that order) if the
	...
	public static String getAnyVar(String key) {
		...
		if (v == null)
			v = System.getenv(keyUp);
		if (v == null)
			v = System.getProperty(keyUp);
		if (v == null)
			v = System.getenv(keyLow);
		if (v == null)
			v = System.getProperty(keyLow);
	...
	/**
	 * <p>
	 * Turns the specified message into HTML code, for use with JLabels, tooltips
	 * and such, to achieve multiline text.
	 * </p>
	 * 
	 * <pre>
	 * {@literal
	...
	/**
	 * Calculates the effective display area of the specified device, ie. the screen
	 * area without stuff like system task bars, etc.
	 * 
	 * @param gd the graphics device to get the effective display area for. If null,
	 *           the default screen device is used.
