...
public class Utils {
	...
	/**
	 * Zooms in the view for the specified display so that the view encompasses all
	 * items in the specified group, with the given margin from the display's edges.
	 * Animated over the specified duration.
	 * 
	 * @param display  the display to zoom
	 * @param group    the group of items to zoom in to
	 * @param margin   the bounds that should be visible in the Display view
	 * @param duration the duration of an animated transition. A value of zero will
	...
	public static void fitToBounds(Display display, String group, int margin, int duration) {
		...
		GraphicsLib.expand(bounds, (double) margin + (int) (1 / display.getScale()));
	 ...
	 * 
	 * @param display  the display to reset
	 * @param duration the duration of animated transition
	 */
	...
	public static BufferedImage getDisplaySnapshot(Display dis) {
		try {
			...
			BufferedImage img = extracted(dis);
	...
	private static BufferedImage extracted(Display dis) {
		if (!GraphicsEnvironment.isHeadless()) {
			try {
				return (BufferedImage) dis.createImage(dis.getWidth(), dis.getHeight());
			} catch (Exception e) {
				// not needed
			}
		}
		return new BufferedImage(dis.getWidth(), dis.getHeight(), BufferedImage.TYPE_INT_RGB);
	}
	 ...
	 * 
	 * @param log    the logger to print the result to
	 * @param prefix the prefix in the printed message
	 * @param f      the runnable to time
	 */
	public static void timed(Logger log, String prefix, Runnable f) {
		...
		if (log.isTraceEnabled())
			log.trace(String.format("%s: %sms (%sns)", prefix, sw.getTime(), sw.getNanoTime()));
	}
	...
	/**
	 * Returns a rectangle containing the smallest and largest values for each
	 * dimension in the specified node.
	 * 
	 * @param node the node for which the extents are to be computed
	 * @param dimX index of the data dimension imaged on the X axis
	 * @param dimY index of the data dimension imaged on the Y axis
	 ...
	 * @param sourceMin   the minimum source value
	 * @param sourceMax   the maximum source value
	 * @param targetMin   the minimum target value
	 * @param targetMax   the maximum target value
	 * @return the source value mapped to the target range
	 */
	public static double normalize(double sourceValue, double sourceMin, double sourceMax, double targetMin,
			double targetMax) {
	...
	/**
	 * Merges the two arrays passed in argument into a single one, in the order
	 * specified.
	 * 
	...
	/**
	 * Runs the main method in the specified class in a subprocess. This method does
	 * not wait for the subprocess to terminate.
	 * 
	 * Source:
	 * https://stackoverflow.com/questions/636367/executing-a-java-application-in-a-separate-process
	 * 
	 * @param clazz the class to start
