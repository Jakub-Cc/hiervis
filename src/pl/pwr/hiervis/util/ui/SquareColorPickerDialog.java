...
/**
 * A dialog window for color selection. Because the default Swing component for
 * color selection is ugly as sin.
 * 
...
public class SquareColorPickerDialog extends JDialog {
	...
	private transient HSV tempSelection = null;
	private transient HSV finalSelection = null;
	...
	public void setSelection(HSV hsv) {
		...
		shadePicker.setSelection(tempSelection.getSaturation(), tempSelection.getValue());
	...
	private void createPickerPanel() {
		...
		getContentPane().add(shadePicker,
				builder.insets(hueInset, 5, hueInset, 5).anchorNorthWest().position(0, 0).build());
		getContentPane().add(huePicker, builder.insets(5).anchorNorthWest().position(1, 0).build());

		huePicker.setPreferredSize(
				new Dimension(20 + 2 * huePicker.getIndicatorSize(), 300 + huePicker.getIndicatorSize()));
	...
	private void createInputFields(JPanel container) {
		...
		cInputFields.add(new JSeparator(),
	...
	private void update(DocumentEvent e) {
		...
		if (d.equals(txtH.getDocument()) || d.equals(txtS.getDocument()) || d.equals(txtV.getDocument())) {
		...
		} else if (d.equals(txtR.getDocument()) || d.equals(txtG.getDocument()) || d.equals(txtB.getDocument())) {
