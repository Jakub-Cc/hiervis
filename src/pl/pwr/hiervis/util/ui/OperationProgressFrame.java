...
/**
 * A simple dialog that can report progress of any operation that can quantify
 * its progress as an integer from range [0, 100].
 * 
...
public class OperationProgressFrame extends JDialog {
	...
	private transient Supplier<Integer> progressCallback;
	private transient Supplier<String> statusCallback;
	...
	public OperationProgressFrame(Window owner, String title) {
		...
		getContentPane().add(progressBar,
				builder.insets(0, 5, 5, 5).position(0, 1).fillHorizontal().anchorNorth().build());
	 ...
	 * 
	 * @param abortOperation the operation to perform when the 'Abort' button is
	 *                       pressed. If null, the button is disabled.
	 */
	...
	/**
	 * Sets the callback that supplies an integer representing the operation's
	 * progress.
	 * 
	 * @param progressSupplier a callback that provides an integer, which represents
	 *                         the operation's progress. If null, the progress bar
	...
	/**
	 * Sets the interval at which to poll the {@code updateCallback} and update the
	 * progress bar.
	 * 
	 * @param intervalMs the interval, in milliseconds. If value is less than or
	 *                   equal to 0, polling is disabled.
	 */
	...
	/**
	 * Schedules an update, so that the progress bar displays the most recent
	 * progress value. If {@code updateCallback} is not set, this method does
	 * nothing.
	 */
