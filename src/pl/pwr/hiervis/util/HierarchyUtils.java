...
public class HierarchyUtils {
    ...
    /**
     * Merges the two hierarchies into a single, new hierarchy, with the specified merging
     * node id.
     * 
     * @param source the source hierarchy that is to be merged into destination hierarchy
     * @param dest   the destination hierarchy that will receive nodes from source hierarchy
     * @param nodeId id of the node in the destination hierarchy that will serve as the
     *               merging point for the two hierarchies. Nodes from the source hierarchy
     *               will be rebased to have this id as root.
	...
	if (sourceDims != destDims) {
	    throw new IllegalArgumentException(String
		    .format("Cannot merge hierarchies, because they have different feature counts - source: %s, dest: %s", sourceDims, destDims));
	}
	...
	double matrix[][] = new double[source
		.getOverallNumberOfInstances()][((BasicInstance) source.getRoot().getNodeInstances().get(0)).getData().length];
	...
	Hierarchy newHierarchy = clone(source.getMainHierarchy(), useSubtree, null);
	for (Instance instance : newHierarchy.getRoot().getSubtreeInstances()) {
	    double[] data = instance.getData();
	    double[] newData = dropDimensions(data, dimensionNumbers);
	...
	newNames = dropDimensions(names, dimensionNumbers);
	List<Node> nodes = getAllNodes(newHierarchy);
	newHierarchy = new BasicHierarchy(nodes, newNames);

	return new LoadedHierarchy(newHierarchy, source.options);
    }

    public static List<Node> getAllNodes(Hierarchy hierarchy) {
	List<Node> nodes = new LinkedList<>();
	nodes.add(hierarchy.getRoot());
	nodes.addAll(getChildren(hierarchy.getRoot()));
	...
	for (Node childNode : node.getChildren()) {
	    list.addAll(getChildren(childNode));
	}
	return list;
    }

    private static double[] dropDimensions(double[] source, List<Integer> dimensionNumbers) {
	double[] newData = new double[dimensionNumbers.size()];
	/*
	 * int j = 0; for (int i = 0; i < source.length; i++) { if (!dimensionNumbers.contains(i))
	 * { newData[j] = source[i]; j++; } }
	 */

	int k = 0;
	for (Integer d : dimensionNumbers) {
	    newData[k] = source[d];
	    k++;
	}
	return newData;
    ...
    private static String[] dropDimensions(String[] source, List<Integer> dimensionNumbers) {
	String[] newData = new String[dimensionNumbers.size()];
	/*
	 * int j = 0; for (int i = 0; i < source.length; i++) { if (!dimensionNumbers.contains(i))
	 * { newData[j] = source[i]; j++; } }
	 */
    ...
    /**
     * Returns a copy of the specified hierarchy. The returned copy is a flat hierarchy
     * containing all instances from the source hierarchy in the root node.
     * 
	...
	source.getMainHierarchy().getRoot().getSubtreeInstances().forEach(in -> {
	    root.addInstance(new BasicInstance(in.getInstanceName(), Constants.ROOT_ID, in.getData(), in.getTrueClass()));
	});
    ...
    /**
     * Returns a copy of the specified loadedHierarchy and all calculated dimension reduction.
     * The returned copy is a flat hierarchy containing all instances from the source
     * hierarchy in the root node.
     * 
     * @param source the hierarchy to flatten
     * @return flattened hierarchy
     */
    ...
    /**
     * @param h the hierarchy to get the class count map for
     ...
     * 
     * @param root root node of the hierarchy to recompute the class count map for
    ...
    /**
     * Creates a new Hierarchy (a deep copy) that doesn't contain the node with the specified
     * id.
     * 
    ...
    /**
     * Creates a wrapper hierarchy around the specified node. The returned hierarchy is a deep
     * copy of the specified node (and its subtree, if that option is selected)
     * 
     * @param source      the source hierarchy the node belongs to
     * @param node        the node to wrap
     * @param withSubtree whether to include child nodes of the specified node
	...
	if (withSubtree) {
	    Arrays.stream(source.getGroups()).filter(n -> n.getId().startsWith(node.getId())).forEach(n -> nodes.add(clone(n, false)));
	}
	else {
	    Arrays.stream(source.getGroups()).filter(n -> n.getId().equals(node.getId())).forEach(n -> nodes.add(clone(n, false)));
	}
     ...
     * 
     * @param h    the hierarchy to look in
    ...
    /**
     * Creates a sub-hierarchy of the specified {@link Hierarchy}, which contains the
     * specified node as root, and all its child nodes. The newly created hierarchy is a deep
     * copy of the source hierarchy.
     * 
     * @param source     the hierarchy to create the sub-hierarchy from
     * @param nodeId     id of the node within the source hierarchy, which will be the root in
     *                   the sub-hierarchy. Defaults to {@link Constants#ROOT_ID} if null.
     * @param destNodeId the new id that the copied nodes will receive. Defaults to
     *                   {@link Constants#ROOT_ID} if null.
     * @return the sub-hierarchy
     * @throws NoSuchElementException if the hierarchy doesn't contain a node with the
     *                                specified name
     */
    ...
    /**
     * Rebases all nodes and instances in the specified stream from the specified old id to
     * the new id. This method creates copies of all objects (nodes, instances) with the same
     * properties, but changed ids.
     * 
     * The rebased nodes have no parent-child relationships set, requiring a follow-up call to
     * a method that will build them.
     * 
     * @param nodes      stream of nodes to rebase
     * @param oldId      the old id to rebase. Only nodes that contain this substring will be
     *                   rebased.
     * @param newId      the new id to rebase the nodes as
     * @param destNodes  the list to put the rebased nodes in
     * @param useSubtree whether the centroid calculation should also include child nodes'
     *                   instances
     */
    public static void rebase(Stream<Node> nodes, String oldId, String newId, List<BasicNode> destNodes, boolean useSubtree) {
	int prefixSub = oldId.length();

	nodes.filter(n -> n.getId().equals(oldId) || HierarchyBuilder.areIdsAncestorAndDescendant(oldId, n.getId())).forEach(n -> {
	    String id = newId + n.getId().substring(prefixSub);
	    BasicNode rebasedNode = new BasicNode(id, null, useSubtree);

	    n.getNodeInstances().forEach(in -> rebasedNode.addInstance(new BasicInstance(in.getInstanceName(), id,
    ...
    /**
     * Creates a deep copy of the specified hierarchy, creating copies of {@link Node}s and
     * {@link Instance}s that comprise the {@link Hierarchy}.
     * 
     * One exception is that {@link Instance}s are not cloned 100% deeply -- the feature
     * values array is copied by reference, since it is never modified, at least for now.
     * 
     * @param source                 the hierarchy to clone.
     * @param useSubtree             whether the centroid calculation should also include
     *                               child nodes' instances
     * @param nodeInclusionPredicate predicate allowing to select nodes that should be
     *                               included in the cloned hierarchy. Can be null to include
     *                               all nodes.
    ...
    /**
     * Creates a deep copy of the specified node, creating copies of {@link Instance}s that
     * have been assigned to this {@link Node}.
     * 
    ...
    /**
     * Serializes the specified hierarchy into CSV format with the specified settings, and
     * saves it in the specified file with UTF-8 encoding.
     * 
     * @param path              the file to save the hierarchy in
     * @param h                 the hierarchy to serialize
     * @param withAssignClass   whether the serialized form should include assign class
     *                          attribute
     * @param withTrueClass     whether the serialized form should include true class
     *                          attribute
     * @param withInstanceNames whether the serialized form should include instance name
     *                          attribute
     * @param withHeader        whether the serialized CSV should include a header with column
     *                          names
     * @throws IOException if an I/O error occurs.
     */
    public static void save(String path, Hierarchy h, boolean withAssignClass, boolean withTrueClass, boolean withInstanceNames, boolean withHeader)
	    throws IOException {
    ...
    /**
     * Serializes the specified hierarchy into CSV format, with the specified additional
     * options.
     * 
     * @param h                 the hierarchy to serialize
     * @param withAssignClass   whether the serialized form should include assign class
     ...
     */
    public static String toCSV(Hierarchy h, boolean withAssignClass, boolean withTrueClass, boolean withInstanceNames, boolean withHeader) {
	int capacity = 0;
	try {
    ...
    /**
     * @param h the hierarchy to get the instance from
    ...
    /**
     * @param h the hierarchy to get the feature count from
     ...
     * hierarchy, given the specified options. If the required capacity overflows range of
     * 32-bit integer, this method throws an {@link ArithmeticException}.
     * 
     * @param h                 the hierarchy to serialize
     * @param withAssignClass   whether the serialized form should include assign class
     *                          attribute
    ...
    /**
     * Constructs a CSV file header for the specified hierarchy and settings, and appends it
     * to the buffer without a newline at the end.
     */
    private static void appendCSVHeader(StringBuilder buf, Hierarchy h, boolean withAssignClass, boolean withTrueClass, boolean withInstanceNames) {
	if (withAssignClass)
	    buf.append("class;");
	if (withTrueClass)
    ...
    /**
     * Constructs a CSV file row for the specified instance and settings, and appends it to
     * the buffer without a newline at the end.
     */
    private static void appendInstance(StringBuilder buf, Instance instance, boolean withAssignClass, boolean withTrueClass,
	...
	buf.append(Arrays.stream(instance.getData()).mapToObj(d -> Double.toString(d)).collect(Collectors.joining(";")));
    }
}
