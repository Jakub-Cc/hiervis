...
public class DimensionReductionnDialog<T extends DimensionReductionI> extends JDialog {
	...
	public static void main(String[] args) throws Exception {
		// FeatureSelectionDialog<FeatureExtraction> f = new
		// FeatureSelectionDialog<>(new
		// StarCoordinates());
		// FeatureSelectionDialog<FeatureExtraction> f = new
		// FeatureSelectionDialog<>(new Tsne());
		// FeatureSelectionDialog<FeatureSelection> f = new FeatureSelectionDialog<>(new
		// InfiniteFS());
		DimensionReductionnDialog<FeatureExtraction> f = new DimensionReductionnDialog<>(new MultidimensionalScaling());
	...
	private JPanel getMainPanel() throws UndefinedParamException {
		...
		for (int i = 0; i < parametersNumber; i++) {
			...
			mainPanel.add(parameterNames[i],
					getLabelConst(1, 1 + rowPerParameter * i, functionParameters.get(i).getValueClass()));
			mainPanel.add(parameterValueGiver[i],
					getValueConst(0, 1 + rowPerParameter * i, functionParameters.get(i).getValueClass()));
			mainPanel.add(parameterDescriptions[i],
					getDesConst(0, 2 + rowPerParameter * i, functionParameters.get(i).getValueClass()));
		}
	...
	private JComponent createDoubleSpinner(DoubleType doubleType) {
		...
		if (doubleType.getDefaultValueF() != null) {
			initialValue = doubleType.getDefaultValueF().function(new Double(pointsAmount),
					new Double(inputDimensions));
		} else
			initialValue = (max - min) / 2 + min;
	...
	private void validateValues(double min, double value, double max) throws IllegalArgumentException {
		if (min > max)
