...
public class DimensionReductionWrapInstanceVisualizationsFrame extends JFrame {
	...
	private boolean checkIfSeparatorNeeded(int index) {
		...
		if (FeatureExtraction.class.isAssignableFrom(currentClass)
				&& FeatureExtraction.class.isAssignableFrom(previousClass))
			return false;

		if (FeatureSelection.class.isAssignableFrom(currentClass)
	...
	private void afterCalculatedFSelectionSelected(int index) {
		// String featureSelectionName =
		// context.getDimensionReductionMenager().getNames()[index -
		// 1];
	...
	private void onDimensionReductionCalculated(CalculatedDimensionReduction reduction) {
		...
		if (FeatureExtraction.class.isAssignableFrom(reduction.dimensionReduction.getClass())
				&& reduction.outputHierarchy != null) {
			fExtractionCalculated(reduction);
		} else if (FeatureSelection.class.isAssignableFrom(reduction.dimensionReduction.getClass())
				&& reduction.fsResult != null) {
