...
public class Tsne extends FeatureExtraction {
	...
	functionParameters.add(FunctionParameters.CreateFunctionParameter("Perplexity",
		...
		new DoubleType((points, dimensions) -> 0.5,
			(points, dimensions) -> (new Double(Math.round((points - 1) / 3))),
			(points, dimensions) -> Math.max(0.5, Math.min(20.0, Math.round((points - 1) / 3))))));
	...
	functionParameters.add(FunctionParameters.CreateFunctionParameter("Paraller calculations",
		"Perform parraller calculations using multithreading.", new BooleanType(true)));
