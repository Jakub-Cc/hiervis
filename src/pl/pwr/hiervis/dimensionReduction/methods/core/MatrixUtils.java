...
public class MatrixUtils {
    ...
    /*
     * result[m,n]=func(matrix[n,m] , vector [n]) performes matrix'*diag(vector) if function
     * is (a,b)->a*b
     */
	...
	for (int i = 0; i < matrix.length; i++) {
	    for (int j = 0; j < matrix[i].length; j++) {
		if (Double.isNaN(matrix[i][j]) || matrix[i][j] == Double.NEGATIVE_INFINITY || matrix[i][j] == Double.POSITIVE_INFINITY) {
		    matrix[i][j] = valueToPut;
		}
	    }
	}
    ...
    /**
     * Using apache.commons-math3 matrix implementation to wrap given matrixes to perform
     * multiplication more efficient on big matrixes
     */
